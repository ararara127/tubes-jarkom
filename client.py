...
def http_request(host, port, path="/", save_as=None, open_browser=False):
    """Mode TCP (HTTP) untuk single client & browser mode."""
    addr = (host, port)
...
def http_multi_client(host, port, path="/", num_clients=5):
    """Menjalankan beberapa client HTTP paralel (default 5)."""
    threads = []
...
def udp_qos_test(
    ...
    """
    Kirim paket UDP ke Proxy atau Server untuk ukur QoS:
    throughput, latency (RTT), jitter, packet loss.
    """
    addr = (host, port)
    logging.info(
    ...
    for seq in range(1, num_packets + 1):
        ...
        # tunggu echo, tapi tidak blocking total (biar tetap jalan intervalnya)
        try:
            ...
            if resp_seq in send_times:
                ...
                recv_count += 1
                logging.info(
                    f"Recv echo seq={resp_seq} from {server_addr}, RTT={rtt*1000:.2f} ms"
                )
    ...
    if rtt_list:
        ...
        # jitter = rata-rata |delay(i) - delay(i-1)|
        diffs = [
    ...
    # throughput berdasarkan total bytes diterima
...
def main():
    parser = argparse.ArgumentParser(description="Client for HTTP and QoS test")
    subparsers = parser.add_subparsers(dest="mode", required=True)

    # HTTP single & browser mode
    ...
    # HTTP multi client (5 clients)
    p_multi = subparsers.add_parser("http-multi", help="Multi-client HTTP (parallel)")
    ...
    p_udp.add_argument("--num", type=int, default=50, help="Number of packets")
    p_udp.add_argument("--size", type=int, default=100, help="Packet size (bytes)")
    p_udp.add_argument("--interval", type=float, default=0.05, help="Interval between packets (s)")
    p_udp.add_argument("--csv", help="Save RTT data to CSV file")
